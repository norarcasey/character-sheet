...
class SetAbilityScores extends Component {
  ...
  render() {
    let totalPointCost = this.props.abilityScores
      .map(ability => this.state.pointCosts[ability.score.toString()])
      .reduce(this.getSum);
          ...
          {this.props.abilityScores.map((ability, index) => {
            ...
            return (
              <li key={index}>
                ...
                <button
                  onClick={() => {
                    this.updateAbilityScore(ability, 1);
                  }}
                >
                  +
                </button>
                <span className={raceBonus > 0 ? 'ability-score-bonus' : ''}>
                  {ability.score + raceBonus}
                </span>
                <button
                  onClick={() => {
                    this.updateAbilityScore(ability, -1);
